...
app.all('*', (req, res) => {
    ...
    if (req.path === '/_ping') { return res.end(''); }
    ...
    if (req.headers.connection === 'Upgrade') {
        ...
        req.on('data', data => { if (socketFound) { return; } proxyRequest.write(data); });
        proxyRequest.on('upgrade', (proxyRes, socket, head) => {
            ...
            res.flushHeaders();
            req.socket.on('data', data => { socket.write(data); });
            socket.on('data', data => { res.socket.write(data); });
            socket.on('end', () => { proxyRequest.end(); res.end(); req.socket.end(); });
            socket.on('error', e => { console.error(e); proxyRequest.end(); res.end(); });
        });
    } else {
        ...
        req.on('end', () => {
            ...
            const proxyRequest = http.request({
                ...
                proxyRes.on('end', () => {
                    const proxyBody = Buffer.concat(proxyChunks).toString('utf8');
                    try {
                ...
                proxyRes.on('error', e => { console.error(e); res.end(); });
            });
