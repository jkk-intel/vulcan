...
cli.command('build')
    .option(
        '--tag <tag>',
        'image tag for the build set (branch name or run number). If not given, the tag is auto-generated',
    )
    .option(
        '--working-directory <workingDirectory>',
        'change directory to specified dir before running the build',
    )
    ...
    .option(
        '--no-prebuilt',
        'disables build skipping when prebuilt images with the same shasum is detected',
    )
    .option('--no-cache', 'disables docker cache and build all layers')
    .option('--no-pull', 'disables docker pull during docker build')
    .option(
        '--precommit-context <precommitContext>',
        'context metadata for precommit (PR) flow',
    )
    ...
    .action(async (options: BuilderCustomOptions) => {
        if (options.background) {
            ...
            nohupDisown(
                process.argv.filter(
                    a => a !== '--background' && a !== '--wait',
                ),
            )
            return
        }
        if (options.wait) {
            const proc = spawn(`tail`, ['-f', '-n', '+1', 'build.all.log'], {
                stdio: 'inherit',
            })
            proc.on('close', async () => {
            ...
            setInterval(async () => {
                const { file, data } = await getFileContent(
                    'build.all.result.log',
                )
                if (typeof data === 'string') {
                    const { data } =
                        await getFileContent(`build.all.result.log`)
                    return process.exit(data.trim() === 'SUCCESS' ? 0 : 1)
                }
            }, 1000)
            return
        ...
        options.log = (str: string) => {
            console.log(str)
            options.logStream.write(str + '\n')
        }
        options.error = (str: string) => {
            console.error(str)
            options.logStream.write(str + '\n')
        }
        options.warn = (str: string) => {
            console.warn(str)
            options.logStream.write(str + '\n')
        }
        const exit = async (code = 0) => {
            ...
            if (!options.logStream.closed) {
                options.logStream.close()
            }
        ...
        try {
            ...
            for (const optionName of Object.keys(options)) {
                ...
                options.log(
                    colors.gray(
                        `   ${optionName}: ${colors.cyan(options[optionName])}`,
                    ),
                )
            }
            ...
            if (fileErrors) {
                for (const err of fileErrors) {
                    options.error(colors.red(err.e))
                }
                return exit(1)
            }
            ...
            const hashCalculationErrors = await calculateComponentHashes(
                options,
                compoMap,
                buildGroups,
            )
            if (hashCalculationErrors) {
                for (const err of hashCalculationErrors) {
                    options.error(colors.red(err.e))
                }
                return exit(1)
            }
            if (!options.tag) {
                ...
                options.warn(
                    colors.yellow(
                        `WARNING; --tag option was not given for the build, using temporary tag '${options.tag}'`,
                    ),
                )
            }
            const buildErrors = await buildAllGroups(
                options,
                compoMap,
                buildGroups,
                configChain,
...
diff.command('show')
    .option(
        '--count',
        'return the number of changed files instead of showing the list',
    )
...
diff.command('match')
    ...
    .option(
        '--working-directory <workingDirectory>',
        'change directory to specified dir for the command',
    )
    .option(
        '--count',
        'return the number of matched files instead of showing the list',
    )
    ...
    .action(
        async (
            ...
            const files =
                code === 0 ? stdout.split('\n').filter(a => a.trim()) : []
            const matchedFiles = await matchFiles(
                files,
                patterns,
                options.exclude,
            )
...
const globhash = cli
    ...
    .option(
        '--context <context>',
        'context regarding this hashing (used as front salt)',
    )
    .option(
        '--info',
        'show information in stderr regarding the contributing files to the hash',
    )
    ...
    .action(
        async (
            ...
            const hash = await calculateGlobHash(
                context,
                patterns,
                options.exclude,
            ...
            if (options.info) {
                console.error(
                    hash.info
                        .map(info => `${info.hash.slice(0, 24)} ${info.file}`)
                        .join('\n'),
                )
                console.error('')
            }
...
picker
    ...
    .option(
        '--natural',
        'natural number counting; instead of [0-m) range, do [1-m]',
    )
    ...
    .action(
        async (
            modulo: string,
            input: string,
            options: { natural: boolean; add: string },
        ) => {
            if (!input || input === 'random') {
                ...
                if (code !== 0) {
                    input = uuidv4().split('-').join('')
                    console.error(
                        colors.red(`Unable to get commit sha: ${stderr}`),
                    )
                    process.exit(1)
                }
                input = stdout.trim()
            ...
            const addApplied = !isNaN(parseInt(options.add, 10))
                ? naturalApplied + parseInt(options.add, 10)
                : naturalApplied
